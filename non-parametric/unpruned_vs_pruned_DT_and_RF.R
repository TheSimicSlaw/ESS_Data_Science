...
# Add region labels
central_2$region <- "Central Europe"
eastern_2$region <- "Eastern Europe"
nordics_2$region <- "Nordics"
southern_2$region <- "Southern Europe"
uk_2$region <- "UK"
west_central_2$region <- "Western Central Europe"

# Combine all datasets
all_data <- bind_rows(
  central_2, eastern_2, nordics_2,
  southern_2, uk_2, west_central_2
)
...
# Prepare count data (summarized)
count_data <- all_data %>%
  group_by(ppltrst, dem_imp) %>%
  summarise(count = n()) %>%
  ungroup()

# Plot with counts
ggplot(count_data, aes(x = ppltrst, y = count, fill = dem_imp)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_text(aes(label = count), 
            position = position_dodge(width = 0.9), 
            vjust = -0.3, size = 3) +
  labs(title = "Distribution of People Trust by Democracy Importance",
       x = "People Trust (ppltrst)", y = "Count", fill = "Democracy Importance") +
  theme_minimal()


#----------------
count_data <- all_data %>%
  group_by(region, ppltrst, dem_imp) %>%
  summarise(count = n()) %>%
  ungroup()
...
ggplot(count_data, aes(x = ppltrst, y = count, fill = dem_imp)) +
  ...
  geom_text(aes(label = count), 
            ...
            vjust = -0.3, size = 2.5) +
  facet_wrap(~ region) +
  labs(title = "People Trust by Democracy Importance Across Regions",
       x = "People Trust (ppltrst)", y = "Count", fill = "Democracy Importance") +
  theme_minimal()
...
# ----- 
predictors <- c("pplfair", "pplhlp", "ppltrst", "trstprl", 
                "trstlgl", "trstplc", "trstplt", "trstprt", "trstsci")

for (var in predictors) {
  p <- ggplot(all_data, aes_string(x = var, fill = "dem_imp")) +
    geom_bar(position = "dodge") +
    facet_wrap(~ region) +
    labs(title = paste("Distribution of", var, "by Democracy Importance and Region"),
         x = var, y = "Count", fill = "Democracy Importance") +
    theme_minimal()
